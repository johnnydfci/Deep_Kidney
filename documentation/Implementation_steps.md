...
```pip3 install -r requirements.txt``` # Once you change directories into the root path of the project
 ...
 ```file_op_to_infer_by_nnunet.ipynb```  # prepare data into the required file name, where each nifty image name has to end with '0000.nii.gz'
...
-   ```python nnunet/dataset_conversion/66_liver-plain.py```   #   prepare data into the required format of nnunet 2/2 step
